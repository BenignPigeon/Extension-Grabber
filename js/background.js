...
//main code -light mode [main extension settings]
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.action === "generateLightLinks") {
    chrome.management.getAll(function(extensions) {
      ...
      htmlContent += '<style>';
      htmlContent += 'body { font-family: Arial, sans-serif; background-color: #F8F8F8; color: #0B0B09; padding: 20px; margin-right: 10px; }';
      ...
      htmlContent += '.extension-item { width: calc(20% - 50px); padding: 20px; background-color: #5CBBFF; border-radius: 10px; border: 1px solid #0052cc; min-height: 120px; }';
      htmlContent += '.extension-title { color: #000000; text-align: center; }';
      htmlContent += '.extension-number { display: inline-block; width: 24px; height: 24px; background-color: #F8F8F8; color: #0052cc; border-radius: 50%; text-align: center; line-height: 24px; font-weight: bold; margin-right: 10px; }';
...
//main code -dark mode [main extension settings]
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.action === "generateDarkLinks") {
    chrome.management.getAll(function(extensions) {
      extensions.sort(function(a, b) {
        var nameA = a.name.toUpperCase();
        var nameB = b.name.toUpperCase();
